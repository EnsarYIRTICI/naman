  ...
  return (
    <>
      ...
      <section className="dhow">
        ...
        <p>
          Ürün kodu 7 hane. İlk 3 hane <b>290</b> ise kasiyer bu öneki ve
          ardından gelen anlamsız (baştaki) sıfırları atlayıp sadece geri kalan
          anlamlı haneleri girer. Bu haneler <mark>sarı</mark> ile
          vurgulanmıştır. Örnekler: 2900027 → <b>27</b> · 2900063 → <b>63</b> ·
          2905083 → <b>5083</b> (baştaki hane sıfır değilse tamamı girilir). Kod
          290 ile başlamıyorsa (293, 282, 300 vb.) tam kod olduğu gibi girilir,
          vurgu yapılmaz.
        </p>
      </section>
      ...
      {groups.map(([group, items]) => (
        <section className="dgroup" data-cat={getCategory(group)} key={group}>
          <h2>
            {group} <small>({items.length} ürün)</small>
          </h2>
          <div className="dcols">
            {items.map((p) => (
...
export default function DocView() {
  ...
  useEffect(() => {
    setData(null);
    setErr("");
    fetch(`/api/public/doc${v ? `?v=${encodeURIComponent(v)}` : ""}`, {
      cache: "no-cache",
    })
      .then((r) => {
        if (!r.ok) throw new Error("HTTP " + r.status);
        return r.json() as Promise<DocPublic>;
      })
      .then(setData)
  ...
  const hrefFor = (label: string, isCurrent: boolean) =>
    isCurrent ? base : `${base}?v=${encodeURIComponent(label)}`;
  ...
  return (
    <div className="doc">
        ...
        <div className="dactions no-print">
          ...
          {sel && (
            <>
              ...
              >
                {sel.label}
                {sel.current && <span className="dchip">Güncel</span>} ▾
              </button>
              {sel.kind === "snapshot" ? (
                <button
                  type="button"
                ...
                <a className="dbtn" href={sel.pdfUrl ?? "#"} download>
                  PDF indir
                </a>
              )}
            </>
      ...
      {data && !sel && (
        <div className="dempty">Henüz yayınlanmış bir sürüm yok.</div>
      )}
      ...
      {sel && !sel.current && (
        <div className="dwarn no-print" role="status">
          ...
          <a
            href={hrefFor(
              data!.versions.find((x) => x.current)?.label ?? "",
              true,
            )}
          >
            Güncel sürüme dön
      ...
      {sel?.kind === "snapshot" && sel.snapshot && (
        <Snapshot snap={sel.snapshot} />
      )}
      {sel?.kind === "pdf" && sel.pdfUrl && (
        <div className="no-print">
          <iframe
            src={sel.pdfUrl}
            title={`Kodlar ${sel.label}`}
            className="dpdf"
          />
          <p className="dnote">
            <a href={sel.pdfUrl} target="_blank" rel="noopener">
              PDF&apos;i yeni sekmede aç
            </a>{" "}
            (telefonda tüm sayfalar görünmüyorsa)
          </p>
        </div>
      )}
      ...
      {sel?.kind === "snapshot" && sel.publishedAt && (
        <p className="dnote">
          Sürüm {sel.label} · {fmtDate(sel.publishedAt)}
          {sel.note ? ` · ${sel.note}` : ""}
        </p>
      )}
      ...
      {dlg && data && (
        <div
          className="dmodal no-print"
          onClick={(e) => e.target === e.currentTarget && setDlg(false)}
          role="dialog"
          aria-label="Sürümler"
        >
          <div className="dmodal-box">
            ...
            {data.versions.map((x) => (
              <a
                key={x.label}
                className={"dver" + (sel?.label === x.label ? " on" : "")}
                href={hrefFor(x.label, x.current)}
              >
                <span className="dver-top">
                  <b>{x.label}</b>
                  {x.current && <span className="dchip">Güncel</span>}
