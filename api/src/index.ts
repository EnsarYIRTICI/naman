...
async function main() {
  ...
  if ((await auth.userCount()) === 0) {
    ...
    if (config.ADMIN_USERNAME && config.ADMIN_PASSWORD && validateUsername(u) && !err) {
      ...
      console.warn(
        "UYARI: Hiç kullanıcı yok, /admin'e kimse giriş yapamaz. .env içinde ADMIN_USERNAME/ADMIN_PASSWORD verin " +
  ...
  const app = buildApp({ config, pool, auth, version });
  const server = app.listen(config.PORT, config.HOST, () => {
    console.log(`Naman API v${version.version}${version.commit ? " (" + version.commit + ")" : ""} ${config.HOST}:${config.PORT} adresinde çalışıyor.`);
  });
